...
object KafkaToFileApp {
  def main(args: Array[String]): Unit = {
    ...
    val fileSink: FileSink[String] = 
      FileSink.forRowFormat(new Path("/usr/local/flink/output.txt"), new SimpleStringEncoder[String]("UTF-8"))
    ...
    env.execute("KafkaToFileApp") // needed to avoid the No Job Found error
  }
